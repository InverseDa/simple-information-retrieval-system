...
# 获取用户的主目录以及项目位置
PATH = os.environ.get('HOME') + '/Information/backend'
...
username.send_keys("393058")
passwd = browser.find_element('id', 'password')
passwd.send_keys("Miao6736369", Keys.RETURN)
for department in ['党政办公室', '教务部', '招生办公室', '研究生院', '科学技术部']:
    ...
    search_box.click()
    # 导入最近40篇文章
    table = browser.find_element('xpath', '/html/body/table/tbody/tr[2]/td/table/tbody/tr[3]/td/table/tbody/tr[3]/td')
    table_html = table.get_attribute('innerHTML')
    article_url = re.compile(r'<a target="_blank" class="fontcolor3" href="(?P<url>.*?)">', re.S).finditer(table_html)
    for i, article in enumerate(article_url):
        ...
        a_url = url + id
        browser.get(a_url)
        content = browser.find_element('xpath', '/html/body/table/tbody/tr[2]/td/table/tbody/tr[3]/td/table/tbody/tr[2]/td')

        # 写入文件 /data/departmentName/title.txt
        f = open(PATH + "/pages/" + str(department) + '_' + str(i) + '.txt', 'a')
